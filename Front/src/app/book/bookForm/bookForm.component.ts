...
export class BookFormComponent implements OnInit {
  ...
  constructor(
    ...
    if (!this.isAddForm) {
      ...
      this.authorName.setValue(this.author.authorName);
      this.publisherName.setValue(this.publisher.publisherName);
    }
  ...
  add(event: MatChipInputEvent): void {
    ...
    const category = this.listCategos.find(c => c.bookCategoName === value);
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    this.categos.push(event.option.value);
  ...
  async onSubmit() {
    if (this.isAddForm) {
      ...
      const result = await this.bookService.uppdateBook(this.bookForm.value as Book)
      if (result == 204) {
